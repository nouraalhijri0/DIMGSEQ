...
class TrainLoop:
    def __init__(
        ...
        *,
        ...
        logger.log('torch.distributed.is_available(): ',torch.distributed.is_available())
        ...
        if th.cuda.is_available(): # DEBUG **
            ...
            logger.log('start DDP')
            self.ddp_model = nn.DataParallel(self.model, device_ids=[g for g in gpu_id])
                '''self.model,
    ...
    def forward_only(self, batch, cond):
        with th.no_grad():
            ...
            for i in range(0, batch.shape[0], self.microbatch):
                ...
                logger.log(f'validating on batch:{i}, \nloss:{losses}\n')
    ...
    def forward_backward(self, batch, cond):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            micro_cond = {
                k: v[i : i + self.microbatch].to(micro.device)
                for k, v in cond.items()
            }
            last_batch = (i + self.microbatch) >= batch.shape[0]
            ...
            if self.use_fp16:
                ...
                loss.backward()
            logger.log(f'training on batch:{i}, \nloss:{losses}\n')
