...
if __name__ == '__main__':
    ...
    for path in files:
        ...
        with open(path, 'r') as f:
            ...
            for row in f:
                ...
                bleu.append(get_bleu(recover, source))
                rougel.append(rougeScore(recover, source)['rougeL_fmeasure'].tolist())
        ...
        #P, R, F1 = score(recovers, source, model_type='microsoft/deberta-xlarge-mnli', lang='en', verbose=True)
    ...
    if len(files)>1:
        if not args.mbr:
            ...
            for (source, recover) in zip(sources, recovers):
                bleu.append(get_bleu(recover, source))
                rougel.append(rougeScore(recover, source)['rougeL_fmeasure'].tolist())
                avg_len.append(len(recover.split(' ')))
                dist1.append(distinct_n_gram([recover], 1))
