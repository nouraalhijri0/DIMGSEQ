...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if self.predict_xstart:
            pred_xstart,_ = process_xstart(model_output)
        else:
        ...
        temp = th.zeros(pred_xstart.shape)
        temp[:,:model_log_variance.size(1),:] = model_log_variance
        model_log_variance = temp.cuda()
        temp[:,:x.size(1),:] = x
        x = temp.cuda()
    ...
    def p_sample(
        ...
        if top_p is not None and top_p > 0:
            ...
            noise = th.randn_like(x)
        temp = th.zeros(out["mean"].shape)
        temp[:,:noise.size(1),:] = noise
        noise = temp.cuda()
        nonzero_mask = (
        ...
        if mask == None:
            ...
            temp = th.zeros(out["mean"].shape)
            temp[:,:x_start.size(1),:] = x_start
            x_start = temp.cuda()
            temp[:,:mask.size(1),:] = mask
            mask = temp.cuda()
    ...
    def _token_discrete_loss(self, x_t, get_logits, input_ids, mask=None, truncate=False, t=None):
        ...
        logger.log(f'x_t: {x_t.shape}, get_logits: {logits.shape}, input_ids: {input_ids.shape},')
        logger.log(f'x_t: {x_t.dtype}, get_logits: {logits.dtype}, input_ids: {input_ids.dtype},')
        # print(logits.shape)
    ...
    def training_losses_seq2seq(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        """
        x_start_fix = x_start # save the orignal true x_0 [64, 128, 128] 
        assert 'input_ids' in model_kwargs
        input_ids_x = model_kwargs.pop('input_ids').to(t.device) #[64, 128] words tokens
        input_ids_mask = model_kwargs.pop('input_mask').to(t.device) #[64, 128]
        x_start_mean = model.model.module.get_embeds(input_ids_x) #[64, 128, 128] x0 without noise
        std = _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod,
                                   th.tensor([0]).to(x_start_mean.device),
                                   x_start_mean.shape) #[64, 128, 128]
        # print(std.shape, )
        # x_start_log_var = 2 * th.log(std)
        x_start = self._get_x_start(x_start_mean, std) #[64, 128, 128] x0 with noise
        # print(x_start_mean.shape, x_start.shape)
        if noise is None:
            noise = th.randn_like(x_start) #[64, 128, 128]

        x_t = self.q_sample(x_start, t, noise=noise, mask=input_ids_mask) # reparametrization trick. #[64, 128, 128] q(x_t)

        get_logits = model.model.module.get_logits
        terms = {}

        target = x_start
        model_output_with_img, hidden_state_img_diff = model(x_t, self._scale_timesteps(t), **model_kwargs) #[64, 128, 128] p(x_t-1 | x_t)

        model_output = model_output_with_img[:,:target.size(1),:]
        ...
        t0_mask = (t == 0)
        t0_loss = mean_flat((x_start_mean - model_out_x_start) ** 2) #EMB(w)-f(z1,1)
        terms["mse"] = th.where(t0_mask, t0_loss, terms["mse"])

        # tT_mask = (t == self.num_timesteps - 1)
        out_mean, _, _ = self.q_mean_variance(x_start, th.LongTensor([self.num_timesteps - 1]).to(x_start.device))
        Reg =  mean_flat(out_mean ** 2)
        ...
        #print(f'\n\n\n{hidden_state_img.shape}, {model_output_with_img.shape}\n\n\n')
        img_loss = mean_flat(hidden_state_img_diff ** 2)

        terms["loss"] = terms["mse"] + img_loss + Reg
